...
# Training
def train(g, feats, topo_cor, topo_embedding, topo_lam, lr, epoch, device, lambda_loss, hidden_dim, sample_size=10,moving_average_decay=0.0):
    ...
    for i in tqdm(range(epoch)):
        ...
        loss, node_embeddings = GNNModel(g, feats, neighbor_dict, topo_cor,topo_embedding,topo_lam,device=device)
        opt.zero_grad()
        loss.backward()
        print(i, loss.item())
        opt.step()
...
def train_new_datasets(dataset_str, epoch_num = 10, lr = 5e-6, lambda_loss=1, sample_size=10, hidden_dim=None,moving_average_decay=0.0):
    x,edge_index,g, labels, split_lists = utils.read_real_datasets(dataset_str)
    x = x.to(device)
    edge_index = edge_index.to(device)
    
    if args.topo_method == "GraRep":
                topo_embedding = get_GraRep_topo_embedding(x.clone().to(torch.device('cpu')),
                                                       edge_index.clone().to(torch.device('cpu')),
                                                       dimensions = args.topo_demension, 
                                                       iterations = args.topo_iter, 
                                                       order = args.topo_order, 
                                                       seed = args.topo_seed).to(device)
    else:
                topo_embedding = get_PPR_topo_embedding(x.clone().to(torch.device('cpu')),
                                                       edge_index.clone().to(torch.device('cpu')),
    ...
    #pre-training
    node_embeddings, loss = train(g, node_features, feat_cor,topo_cor, args.topo_lam, lr=lr, epoch=epoch_num, device=device,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--topo_method', type=str, default="GraRep")
    parser.add_argument('--lr', type=float, default=5e-6)
    parser.add_argument('--epoch_num', type=int, default=50)
    
    parser.add_argument('--topo_demension', type=int, default=16)
    parser.add_argument('--topo_iter', type=int, default=5)
    parser.add_argument('--topo_order', type=int, default=20)
    parser.add_argument('--topo_seed', type=int, default=42)
    parser.add_argument('--topo_lam', type=float, default=0.0001)
